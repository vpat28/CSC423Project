...
query = """
    ...
    phoneNum TEXT CHECK(length(phoneNum)==10) NOT NULL UNIQUE,
    managerStaffNo INT CHECK(managerStaffNo>0),
    PRIMARY KEY(clinicNo)
    /*FOREIGN KEY (managerStaffNo) REFERENCES staff(staffNo)ON DELETE CASCADE*/
    );
    """
cursor.execute(query)
query = """
 CREATE TABLE Owner(
    ownerNo  INT CHECK(ownerNo >0),
    name    TEXT,
    address TEXT,
    ownerPhone TEXT CHECK(length(ownerPhone)=10) UNIQUE,
    clinicNo INT,
    PRIMARY KEY(ownerNo),
...
name TEXT,
address TEXT,
staffPhone TEXT CHECK(length(staffPhone)=10) NOT NULL UNIQUE,
DOB        TEXT CHECK(DOB < "2004-01-01"),
position TEXT,
...
chiefComplaint TEXT,
date   TEXT CHECK(date <= "2022-12-08"),
actionTaken TEXT,
petNo INT,
staffNo INT,
PRIMARY KEY(examNo)
